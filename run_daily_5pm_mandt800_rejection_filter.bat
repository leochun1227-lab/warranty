...
set "LOG_FILE=%~dp0logs\ctm_daily_12pm_%LOG_STAMP%.log"

echo ==================================================>> "%LOG_FILE%"
echo CTM V44 daily 12:00 run started at %date% %time%>> "%LOG_FILE%"
echo ==================================================>> "%LOG_FILE%"
...
python -u "%~dp0ctm_v44_history_safe_mandt800_rejection_filter.py" --once --company-file "%~dp0fetch_all_tickets_fast_with_firebase_MANDT800_REJECTION_FILTER.py" >> "%LOG_FILE%" 2>&1
set "ERR=%ERRORLEVEL%"

echo CTM V44 daily 12:00 run finished at %date% %time% with code %ERR%>> "%LOG_FILE%"
exit /b %ERR%
