...
set "LOG_FILE=%~dp0logs\recall_claims_sync_%LOG_STAMP%.log"

echo ==================================================>> "%LOG_FILE%"
echo Recall Claims standalone sync started at %date% %time%>> "%LOG_FILE%"
echo ==================================================>> "%LOG_FILE%"

if not exist "%~dp0sync_recall_claims_to_firebase.py" (
  echo ERROR: Missing sync_recall_claims_to_firebase.py>> "%LOG_FILE%"
  exit /b 1
)
...
if "%PYTHON_CMD%"=="py -3" (
  py -3 -u "%~dp0sync_recall_claims_to_firebase.py" >> "%LOG_FILE%" 2>&1
) else (
  "%PYTHON_CMD%" -u "%~dp0sync_recall_claims_to_firebase.py" >> "%LOG_FILE%" 2>&1
)
set "ERR=%ERRORLEVEL%"

echo Recall Claims standalone sync finished at %date% %time% with code %ERR%>> "%LOG_FILE%"
exit /b %ERR%
