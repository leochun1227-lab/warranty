...
if "%PYTHON_CMD%"=="py -3" (
  py -3 -u "%~dp0ctm_v44_history_safe_mandt800_rejection_filter.py" --rebuild-analytics-only
) else (
  "%PYTHON_CMD%" -u "%~dp0ctm_v44_history_safe_mandt800_rejection_filter.py" --rebuild-analytics-only
)
if errorlevel 1 (
  echo.
