...
# Compatibility shim:
# running this legacy file directly now delegates to the current main monitor
# so the backup keeps the same approval/lastUpdate/remove contract.
from ctm_v44_history_safe_mandt800_rejection_filter import main as _current_main
...
def is_approved_by_z1z8(ticket_or_snapshot: Dict[str, Any]) -> bool:
    code = clean(ticket_or_snapshot.get("TicketStatus") or ticket_or_snapshot.get("statusCode") or ticket_or_snapshot.get("StatusCode") or ticket_or_snapshot.get("code")).upper()
    ...
    if code == "Y7" or "approved claims closed" in text:
        return False
...
def calculate_handling_speed(logs: list[Dict[str, Any]], current_critical_rows: list[Dict[str, Any]], ticket_by_id: Optional[Dict[str, Dict[str, Any]]] = None, as_of: Optional[Any] = None) -> Dict[str, Any]:
    ...
    # needs to be processed down to the 300-ticket target. Use recent exited
    # critical capacity as the denominator. New entered-critical tickets are
    # reported separately as pressure, but they should not cancel out the team's
    # demonstrated clear capacity for this KPI.
    ...
    elif clear_capacity_per_day > 0:
        estimated_clear_days = int(math.ceil(current_stock / clear_capacity_per_day))
    else:
    ...
    if tickets_to_clear_to_target <= 0:
        ...
        estimated_clear_to_target_days = int(math.ceil(tickets_to_clear_to_target / clear_capacity_per_day))
    else:
        estimated_clear_to_target_days = None

    net_clear_per_day = clear_capacity_per_day
    ...
    return {
        ...
        "note": "2-week benchmark uses true entered-critical time; if missing, ticket CreatedOn/created date from full ticket snapshot is used. Estimated clear time uses recent last-7-active-days exited-critical capacity. Estimated clear to target uses max(0, current critical - 300) divided by the same exit capacity; entered-critical pressure is reported separately.",
    }
...
def _event_is_unapproved_exit(e: Dict[str, Any], t: Optional[Dict[str, Any]] = None) -> bool:
    ...
    if code == "Y8" or "unapproved" in text:
        return True
    if isinstance(t, dict) and "isApproved" in t:
        return not bool(t.get("isApproved"))
    return False
...
def _build_team_view(
    ...
    weekly_avg_cost: Dict[str, Dict[str, Any]] = {}
    ...
    for e in history_events:
        ...
        if typ == "entered":
            ...
            if not _event_is_unapproved_exit(e, t):
                wk = _week_start_key(dkey)
                if wk:
                    bucket = weekly_avg_cost.setdefault(wk, {"weekStart": wk, "approvedTickets": 0, "approvedAmount": 0.0})
                    bucket["approvedTickets"] += 1
                    bucket["approvedAmount"] += parse_amount(amt)
                daily_bucket = approved_cost_daily.setdefault(dkey, {"approvedTickets": 0, "approvedAmount": 0.0})
                daily_bucket["approvedTickets"] += 1
                daily_bucket["approvedAmount"] += parse_amount(amt)
        elif typ == "moved":
    ...
    def repair_cost_distribution_for_events(events: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        buckets = [
    ...
    handling_speed = calculate_handling_speed(matching_events, critical_now, ticket_by_id, generated_at)
    ...
    for wk in sorted(weekly_avg_cost.keys()):
        row = weekly_avg_cost[wk]
        approved_count = int(row.get("approvedTickets") or 0)
        approved_amount = round(float(row.get("approvedAmount") or 0), 2)
        week_dt = parse_date_any(wk)
        week_end = week_dt.date().fromordinal(week_dt.date().toordinal() + 6).isoformat() if week_dt else wk
        weekly_avg_cost_trend.append({
            "weekStart": wk,
            "weekEnd": week_end,
            "approvedTickets": approved_count,
            "approvedAmount": approved_amount,
            "avgCostPerTicket": round(approved_amount / approved_count, 2) if approved_count else 0,
        })
    ...
    for idx, d in enumerate(dates):
        ...
        if idx % 7 == 0 or idx == len(dates) - 1:
            daily_avg_cost_trend.append({
                "date": d,
                "approvedTickets": int(approved_count_cum),
                "approvedAmount": round(approved_amount_cum, 2),
                "avgCostPerTicket": round(approved_amount_cum / approved_count_cum, 2) if approved_count_cum else 0,
            })
    ...
    for i, start_day in enumerate(dates):
        for end_day in dates[i:]:
            range_events = [e for e in matching_events if start_day <= (event_date_key(e) or "") <= end_day]
            exit_events = [e for e in range_events if event_type(e) == "exited"]
            approved_count = 0
            unapproved_count = 0
            approved_amount_range = 0.0
            unapproved_amount_range = 0.0
            for e in exit_events:
                ...
                if _event_is_unapproved_exit(e, t):
                    unapproved_count += 1
                    unapproved_amount_range += amount
                else:
                    approved_count += 1
                    approved_amount_range += amount
            ...
            summary.update({
                ...
                "approvedTickets": int(approved_count),
                "unapprovedTickets": int(unapproved_count),
                "approvedAmount": round(approved_amount_range, 2),
                "unapprovedAmount": round(unapproved_amount_range, 2),
                "avgCostPerTicket": round(approved_amount_range / approved_count, 2) if approved_count else 0,
                "source": "python pre-calculated arbitrary date range",
            })
            period_snapshots[f"{start_day}|{end_day}"] = {
                "summary": summary,
                "repairCostDistribution": repair_cost_distribution_for_events(exit_events),
    ...
    return {
        ...
        "repairCostDistribution": repair_cost_distribution_for_events(matching_events),
        "dashboardKpis": dashboard_kpis,
...
def write_analytics(source_root: str, monitor_root: str, snap: Optional[Dict[str, Dict[str, Any]]] = None) -> None:
    ...
    payload = {
        ...
        "employee": build_employee_analytics(snap, history_events, ts, employee_directory),
    ...
    analytics_ref.child("team").set(firebase_safe_json(payload["team"]))
...
def compare_and_write(source_root: str, monitor_root: str) -> None:
    ...
    # - no logs are generated for baseline seeding / reconciliation.
