...
def build_dealer_ticket_rows(source_root: str, mapping: Dict[str, Any]) -> list[Dict[str, Any]]:
    ...
    for key, raw in entries:
        ...
        rows.append({
            ...
            "amount": parse_amount(ticket.get("AmountIncludingTax")), "created": created, "agingDays": aging_days(created),
...
def calculate_handling_speed(logs: list[Dict[str, Any]], current_critical_rows: list[Dict[str, Any]], ticket_by_id: Optional[Dict[str, Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    for tid, exit_dt, e in exit_items:
        ...
        buckets[_duration_bucket(days)] += 1
...
def build_employee_analytics(snap: Dict[str, Dict[str, Any]], history_events: list[Dict[str, Any]], generated_at: str) -> Dict[str, Any]:
    ...
    def build_view(label: str) -> Dict[str, Any]:
        ...
        for e in history_events:
            ...
            # audit bucket and exported by inspect_unmapped_removed_events.py.
            total_removed += 1
            if dkey:
...
def _build_team_view(
    ...
    for e in history_events:
        ...
        if typ == "entered":
            ...
            tid = event_ticket_id(e)
            t = ticket_by_id.get(tid, {}) if tid else {}
            exited_value_daily[dkey] = exited_value_daily.get(dkey, 0.0) + parse_amount(t.get("amount") if isinstance(t, dict) else e.get("amount"))
        elif typ == "moved":
    ...
    return {
        ...
        "summary": {
            "totalTickets": len([t for t in snap.values() if ticket_matches(t)]),
        ...
        "handlingSpeed": calculate_handling_speed(matching_events, critical_now, ticket_by_id),
        "daily": {
...
def build_team_analytics(snap: Dict[str, Dict[str, Any]], history_events: list[Dict[str, Any]], generated_at: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "storageNote": "Pre-calculated by Python. Claim buttons switch between pre-built views; browser only renders.",
    }
