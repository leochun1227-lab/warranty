...
DEFAULT_SA_PATH = str(Path.cwd() / "firebase-service-account.json")
...
CLAIM_TYPE_SEARCH_FIELDS = [
    ...
    "Subject", "Name", "Category", "ServiceCategory",
    "WarrantyClaimType", "Warranty Claim Type",
    "Warranty Type",
    "claim", "type",
]
...
def sydney_today_iso() -> str:
    """Return today's date in Australia/Sydney.

    The dashboard is used in Sydney time, so synthetic analytics events should
    not be stamped with the previous UTC day near midnight.
    """
...
def approval_state_storage_key(v: Any) -> str:
    """Stable Firebase key for approvalState rows.

    Do not store approvalState directly under a purely numeric ticket id. RTDB
    clients/Admin SDK can read numeric-key objects back as arrays/lists, which
    made analytics see approvalState as empty and caused Approved/Unapproved KPI
    cards to disappear after rebuild.
    """
    return "ticket_" + (safe_key(v) or "blank")


def approval_state_dict_by_ticket_id(node: Any) -> Dict[str, Dict[str, Any]]:
    """Normalize legacy numeric/list approvalState into ticket_<id> -> record."""
    ...
    for raw_key, val in entries:
        if not isinstance(val, dict):
            continue
        raw_key_s = clean(raw_key)
        tid = clean(val.get("ticketId") or val.get("id") or val.get("TicketID"))
        if not tid and raw_key_s.startswith("ticket_"):
            tid = raw_key_s[len("ticket_"):]
        if not tid:
            tid = raw_key_s
        key = approval_state_storage_key(tid)
        rec = dict(val)
        rec.setdefault("ticketId", tid)
        out[key] = rec
    return out
...
# Approved status texts as they appear on the C4C ticket / SAP report. Used to
...
def ticket_sales_order_number(ticket_or_snapshot: Dict[str, Any]) -> str:
    return normalize_po_number(
        ticket_or_snapshot.get("Sales Order")
        or ticket_or_snapshot.get("SalesOrder")
        or ticket_or_snapshot.get("salesOrder")
        or ticket_or_snapshot.get("LookupSalesOrder")
        or ticket_or_snapshot.get("lookupSalesOrder")
    )


def has_approved_sap_signal(ticket_or_snapshot: Dict[str, Any]) -> bool:
    return bool(ticket_po_number(ticket_or_snapshot) or ticket_sales_order_number(ticket_or_snapshot))


def normalize_ticket_id(v: Any) -> str:
    raw = clean(v)
    if re.fullmatch(r"\d+\.0+", raw):
        raw = raw.split(".", 1)[0]
    return raw
...
def ticket_created_on_day(ticket_or_snapshot: Dict[str, Any]) -> str:
    return date_key(
        ticket_or_snapshot.get("CreatedOn")
        or ticket_or_snapshot.get("Created On")
        or ticket_or_snapshot.get("createdOn")
        or ticket_or_snapshot.get("created")
        or ticket_or_snapshot.get("CreatedAt")
        or ticket_or_snapshot.get("createdAt")
    )


def ticket_claim_approved_on_value(ticket_or_snapshot: Dict[str, Any]) -> str:
    return get_field(ticket_or_snapshot, [
        "ClaimApprovedOnDateTime",
        "ClaimApprovedOnDate",
        "ClaimApprovedOn",
        "Claim Approved On",
        "Claim Approved On DateTime",
        "claimApprovedOnDateTime",
        "claimApprovedOnDate",
        "claimApprovedOn",
        "claimapprovedondate",
...
def ticket_resolved_on_value(ticket_or_snapshot: Dict[str, Any]) -> str:
    return get_field(ticket_or_snapshot, [
        "ResolvedOnDateTime",
        "ResolvedOnDate",
        "ResolvedOn",
...
def ticket_changed_on_value(ticket_or_snapshot: Dict[str, Any]) -> str:
    return get_field(ticket_or_snapshot, [
        "ChangeOnDateTime",
        "ChangeOnDate",
        "ChangeOn",
...
def ticket_posting_date_value(ticket_or_snapshot: Dict[str, Any]) -> str:
    direct = get_field(ticket_or_snapshot, [
        "Posting Date",
        "PostingDate",
        "postingDate",
        "PostingDateTime",
    ...
    if po:
        return clean(sap_posting_date_for_po(po))
    return ""


def ticket_status_code(ticket_or_snapshot: Dict[str, Any]) -> str:
    return clean(ticket_or_snapshot.get("TicketStatus") or ticket_or_snapshot.get("statusCode") or ticket_or_snapshot.get("Status") or ticket_or_snapshot.get("code")).upper()


def ticket_status_text(ticket_or_snapshot: Dict[str, Any]) -> str:
    return clean(ticket_or_snapshot.get("TicketStatusText") or ticket_or_snapshot.get("statusText") or ticket_or_snapshot.get("status")).lower()
...
def is_approved_status(ticket_or_snapshot: Dict[str, Any]) -> bool:
    ...
    if "approved claims closed" in text or ("closed" in text and "approved" in text):
        return False
    return code in APPROVED_C4C_STATUS_CODES or text in APPROVED_C4C_SAP_REPORT_STATUSES


def is_closed_approved_status(ticket_or_snapshot: Dict[str, Any]) -> bool:
    code = ticket_status_code(ticket_or_snapshot)
    text = ticket_status_text(ticket_or_snapshot)
    return code == "Y7" or "approved claims closed" in text
...
def resolved_approved_date(ticket_or_snapshot: Dict[str, Any]) -> str:
    ...
    if approval_decision(ticket_or_snapshot) != "approved":
        return ""
    value, _ = approval_decision_value_and_source(ticket_or_snapshot)
    return date_key(value)


def c4c_approved_decision_date(ticket_or_snapshot: Dict[str, Any]) -> str:
    return resolved_approved_date(ticket_or_snapshot)


def c4c_approved_changed_date(ticket_or_snapshot: Dict[str, Any]) -> str:
    return c4c_approved_decision_date(ticket_or_snapshot)


def approval_decision_value_and_source(ticket_or_snapshot: Dict[str, Any]) -> tuple[str, str]:
    ...
    if decision == "unapproved":
        ...
        value = first_nonempty_value(
            ...
            ticket_or_snapshot.get("unapprovedDate"),
        ...
        if value:
            if resolved_value:
                return value, "resolved_on"
            if changed_value:
                return value, "changed_on"
            return value, "legacy_unapproved_backup"
        return "", ""
...
def approval_decision_time(ticket_or_snapshot: Dict[str, Any]) -> str:
    value, _ = approval_decision_value_and_source(ticket_or_snapshot)
    return clean(value)
...
_SAP_POSTING_DATE_BY_PO_CACHE: Optional[Dict[str, str]] = None
_LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE: Optional[Dict[str, str]] = None


def load_sap_posting_date_index() -> Dict[str, str]:
    """Build a PO -> posting date index from the SAP analytics export."""
    global _SAP_POSTING_DATE_BY_PO_CACHE
    if _SAP_POSTING_DATE_BY_PO_CACHE is not None:
        return _SAP_POSTING_DATE_BY_PO_CACHE

    explicit_path = clean(os.getenv("C4C_SAP_ANALYTICS_CSV_PATH"))
    candidates = [Path(explicit_path)] if explicit_path else []
    # Portable default: keep the SAP export beside this script. This works when
    # the warranty folder is copied to another machine/user profile.
    ...
    for folder in search_dirs:
        if not folder.is_dir():
            continue
        discovered.extend(folder.glob("SAPAnalyticsReport*.csv"))
    candidates.extend(sorted(
        discovered,
        key=lambda p: p.stat().st_mtime if p.exists() else 0,
        reverse=True,
    ))
    path = next((p for p in candidates if clean(str(p)) and p.is_file()), None)
    if path is None:
        _SAP_POSTING_DATE_BY_PO_CACHE = {}
        return _SAP_POSTING_DATE_BY_PO_CACHE
    print(f"[C4C SAP] Using posting date index CSV: {path}")

    posting_by_po: Dict[str, str] = {}
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if not po:
                continue
            posting_day = date_key(val(row, "Posting Date"))
            if not posting_day:
                continue
            current = posting_by_po.get(po)
            if not current or posting_day < current:
                posting_by_po[po] = posting_day

    _SAP_POSTING_DATE_BY_PO_CACHE = posting_by_po
    return _SAP_POSTING_DATE_BY_PO_CACHE


def load_legacy_csv_created_on_index() -> Dict[str, str]:
    """Build a ticket-id -> created-on index from the legacy SAP analytics CSV."""
    global _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE
    if _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE is not None:
        return _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE
    ...
    path = next((p for p in candidates if clean(str(p)) and p.is_file()), None)
    if path is None:
        _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE = {}
        return _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE

    created_by_ticket: Dict[str, str] = {}
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.reader(f)
        try:
            headers = next(reader)
        ...
        def csv_ticket_id(row: list[str]) -> str:
            for key in ("Ticket ID", "Unnamed: 1", "Unnamed: 3", "Ticket", "Ticket Name"):
                v = normalize_ticket_id(val(row, key))
                if v:
                    return v
            return ""
        ...
        for row in reader:
            ...
            if not tid:
                continue
            created_day = date_key(val(row, "Created On"))
            if not created_day:
                continue
            current = created_by_ticket.get(tid)
            if not current or created_day < current:
                created_by_ticket[tid] = created_day

    _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE = created_by_ticket
    return _LEGACY_CSV_CREATED_ON_BY_TICKET_ID_CACHE


def sap_posting_date_for_po(po: Any) -> str:
    po_key = normalize_po_number(po)
    if not po_key:
        return ""
    return load_sap_posting_date_index().get(po_key, "")


def legacy_csv_created_on_for_ticket(ticket_or_snapshot: Dict[str, Any]) -> str:
    tid = normalize_ticket_id(
        ticket_or_snapshot.get("id")
        or ticket_or_snapshot.get("ticketId")
        or ticket_or_snapshot.get("TicketID")
    )
    if not tid:
        return ""
    return load_legacy_csv_created_on_index().get(tid, "")
...
def first_real_date(*values: Any) -> str:
    """Return the first parseable date, treating blanks, # and non-dates as missing."""
    for v in values:
        d = date_key(v)
        if d:
            return d
...
def normalize_claim_text(v: Any) -> str:
    s = clean(v).lower().replace("_", " ").replace("-", " ")
    s = re.sub(r"[^a-z0-9]+", " ", s)
    return " ".join(s.split())


def matches_claim_keyword(search_text: str, keywords: Iterable[str]) -> bool:
...
def ticket_claim_type(ticket: Dict[str, Any]) -> str:
    ...
    values: list[Any] = [get_field(ticket, CLAIM_TYPE_SEARCH_FIELDS)]
    values.extend(ticket.get(field) for field in CLAIM_TYPE_SEARCH_FIELDS if field in ticket)
...
def build_snapshot(source_root: str) -> Dict[str, Dict[str, Any]]:
    ...
    for key, row in entries:
        ...
        approval_decision_value = approval_decision(ticket)
        approval_value, approval_value_source = approval_decision_value_and_source(ticket)
        approval_decision_day = date_key(approval_value)
        claim_approved_on = ticket_claim_approved_on_value(ticket)
        resolved_on = ticket_resolved_on_value(ticket)
        changed_on = ticket_changed_on_value(ticket)
        posting_date = ticket_posting_date_value(ticket)
        ...
        snap[ticket_storage_key(tid)] = {
            ...
            "approvalDecision": approval_decision_value,
            "approvalDecisionDate": approval_decision_day,
            "approvalDecisionTime": clean(approval_value),
            "approvalDecisionSource": approval_value_source,
            "approvalDate": clean(approval_decision_day if approval_decision_value == "approved" else ""),
            "decisionTime": clean(approval_value),
            "decisionDay": approval_decision_day,
            "claimApprovedOn": claim_approved_on,
            "claimApprovedOnDateTime": claim_approved_on,
            "resolvedOn": resolved_on,
            "resolvedOnDateTime": resolved_on,
            "changeOnDateTime": changed_on,
            "changedOn": changed_on,
            "postingDate": posting_date,
            "isApproved": approval_decision_value == "approved",
            "isUnapproved": approval_decision_value == "unapproved",
            "created": ticket.get("CreatedOn") or ticket.get("createdOn") or ticket.get("CreatedAt") or "",
            # lastUpdateDate mirrors the current C4C ChangeOnDateTime so every
            # scan keeps the snapshot aligned even when status itself is unchanged.
            "lastUpdateDate": clean(changed_on),
...
def build_dealer_ticket_rows(source_root: str, mapping: Dict[str, Any]) -> list[Dict[str, Any]]:
    ...
    for key, raw in entries:
        ...
        rows.append({
            ...
            "erpPurchaseOrder": ticket_po_number(ticket), "salesOrder": clean(ticket.get("Sales Order") or ticket.get("SalesOrder") or ticket.get("salesOrder") or ticket.get("LookupSalesOrder") or ticket.get("lookupSalesOrder")), "approvalDecision": approval_decision(ticket), "approvalDecisionDate": approval_decision_date(ticket),
            "approvalDate": clean(c4c_approved_decision_date(ticket)),
...
def build_dealer_analytics(source_root: str, monitor_root: str, history_events: list[Dict[str, Any]], generated_at: str) -> Dict[str, Any]:
    ...
    def build_view_for_dealer(dealer: str, label: str) -> Dict[str, Any]:
        ...
        detail_rows = [] if is_all_dealers else critical_rows
        detail_logs = [] if is_all_dealers else logs
        detail_materials = [] if is_all_dealers else material_groups
        detail_delivered_materials = [] if is_all_dealers else delivered_material_groups
        return {
            "label": label,
            "key": claim_key(label),
            ...
            "deliveredMaterials": detail_delivered_materials,
        }
...
def build_employee_analytics(
    ...
    def approved_history_start_date() -> str:
        ...
        for t in snap.values():
            if not isinstance(t, dict):
                continue
            if approval_decision(t) != "approved":
                continue
            d = approval_decision_date(t)
            if d:
                dates.append(d)
        return min(dates) if dates else DASHBOARD_MIN_DATE

    approval_history_start = approved_history_start_date()
    ...
    return {
        ...
        "approvalHistoryStartDate": all_view.get("approvalHistoryStartDate", DASHBOARD_MIN_DATE),
...
def _event_claim_type(e: Dict[str, Any], ticket_by_id: Dict[str, Dict[str, Any]]) -> str:
    event_type = ticket_claim_type(e)
    if event_type:
        return event_type
    tid = event_ticket_id(e)
    t = ticket_by_id.get(tid, {}) if tid else {}
    return ticket_claim_type(t)
...
def _event_is_approved_exit(e: Dict[str, Any], t: Optional[Dict[str, Any]] = None) -> bool:
    if clean(e.get("bucket")).lower() == "approved":
        return True
...
def add_same_window_initial_unapproved_events(
    ...
    generated_day = sydney_today_iso()
    ...
    for t in (snap or {}).values():
        ...
        synthetic_events.append({
            ...
            "synthetic": True,
            "syntheticReason": "initial-unapproved-first-seen",
...
def _build_team_view(
    ...
    def approved_history_start_date() -> str:
        dates: list[str] = []
        for rec in approval_state.values():
            if not isinstance(rec, dict):
                continue
            d = date_key(rec.get("decisionDay") or rec.get("decisionTime") or rec.get("changedOn"))
            if d:
                dates.append(d)
        for t in snap.values():
            if not isinstance(t, dict):
    ...
    def employee_status_for(name: Any) -> str:
        entry = employee_directory.get(employee_directory_key(name), {})
        return "exited" if clean(entry.get("status")).lower() == "exited" else "active"

    def is_exited_employee_for_team(name: Any) -> bool:
        return employee_status_for(name) == "exited"

    def enrich_approved_source_row(row: Dict[str, Any]) -> Dict[str, Any]:
        out = dict(row)
        name = clean(out.get("name") or out.get("TicketName"))
        matched = ticket_by_name.get(name.lower()) if name else None
        matched_claim_type = ticket_claim_type(matched or {})
        if matched_claim_type:
            out["claimType"] = matched_claim_type
            out["sourceTicketId"] = clean((matched or {}).get("id"))
        else:
            out["claimType"] = ticket_claim_type(out)
        return out
    ...
    def approved_rows_for_range(start: str, end: str) -> list[Dict[str, Any]]:
        ...
        for rec_key, rec in approval_state.items():
            if not isinstance(rec, dict):
                continue
            tid = clean(rec.get("ticketId") or rec.get("id") or rec_key)
            current = ticket_by_id.get(tid, {}) if tid else {}
            merged = {**rec, **current}
            ...
            if not clean(merged.get("claimType")):
                merged["claimType"] = (
                    clean(rec.get("claimType"))
                    or clean(current.get("claimType"))
                    or ticket_claim_type(current or rec)
                )
            if not clean(merged.get("employee")):
                merged["employee"] = clean(rec.get("employee") or current.get("employee"))
            if not clean(merged.get("dealer")):
                merged["dealer"] = clean(rec.get("dealer") or current.get("dealer"))
            merged.update({
                "id": tid,
                "name": clean(current.get("name") or rec.get("ticketName") or rec.get("name")),
                "approvalDecision": "approved",
                "approvalDecisionDate": decision_date,
                "approvalDate": clean(decision_date),
                "isApproved": True,
                "isUnapproved": False,
                "statusText": clean(current.get("statusText") or rec.get("status")),
                "status": clean(current.get("statusText") or rec.get("status")),
                "erpPurchaseOrder": clean(current.get("erpPurchaseOrder") or rec.get("po") or ticket_po_number(merged)),
                "amount": current.get("amount") if current.get("amount") not in (None, "") else rec.get("amount"),
                "created": clean(current.get("created") or rec.get("created")),
                "sourceGroup": clean(rec.get("source")) or "approvalState",
            })
            merged_claim_type = clean(merged.get("claimType"))
            if claim_filter and merged_claim_type and merged_claim_type != claim_filter:
                continue
            if not ticket_matches(merged) and not (claim_filter and not merged_claim_type and clean(merged.get("approvalDecision")).lower() == "approved"):
                continue
            dedupe_key = tid or f"{clean(merged.get('name')).lower()}|{clean(merged.get('erpPurchaseOrder'))}"
            rows_by_key[dedupe_key] = merged
        for t in scoped_tickets:
            if clean(t.get("approvalDecision")) != "approved":
                continue
            current_claim_type = clean(t.get("claimType")) or ticket_claim_type(t)
            if claim_filter and current_claim_type and current_claim_type != claim_filter:
                continue
            if not ticket_matches(t) and not (claim_filter and not current_claim_type):
                continue
            decision_date = approved_reporting_date(t)
            if decision_date and start <= decision_date <= end:
                dedupe_key = clean(t.get("id")) or f"{clean(t.get('name')).lower()}|{clean(t.get('erpPurchaseOrder'))}"
                rows_by_key[dedupe_key] = dict(t)
        return list(rows_by_key.values())

    def approved_reporting_date(t: Dict[str, Any]) -> str:
        return approval_decision_date(t)
    ...
    def unapproved_events_for_range(start: str, end: str) -> list[Dict[str, Any]]:
        return latest_event_by_ticket([
            ...
            if start <= (event_date_key(e) or "") <= end
            and _event_is_unapproved_exit(e, ticket_by_id.get(event_ticket_id(e), {}))
        ])
    ...
    all_approved_rows = approved_rows_for_range(approval_history_start, generated_day)
    ...
    for t in approved_rows_for_range(approval_history_start, generated_day):
        decision_day = approved_reporting_date(t)
    ...
    return {
        ...
        "repairCostDistribution": repair_cost_distribution_for_rows(approved_rows_for_range(approval_history_start, generated_day)),
        "approvalTicketRows": approval_ticket_rows_for_range(approval_history_start, generated_day),
        "employeeApprovalRows": employee_approval_rows_for_range(approval_history_start, generated_day),
        "dashboardKpis": dashboard_kpis,
...
def write_analytics(
    ...
    try:
        approval_state_raw = db.reference(f"{monitor_root}/approvalState").get() or {}
    ...
    if include_dealer:
        ...
        # Do not delete the whole byDealer tree in one request: RTDB rejects large
        # deletes. Each current dealer payload is overwritten below.
    ...
    print(
        ...
        f"dealerAnalytics={'updated' if include_dealer else 'skipped'}"
    )
...
def reset_baseline(
    ...
    try:
        approval_stats = update_approval_state(monitor_root, snap)
        print(f"[RESET] approvalState seeded: newlyApproved={approval_stats['newlyApproved']}, "
              f"newlyUnapproved={approval_stats['newlyUnapproved']}, "
              f"correctedDetected={approval_stats.get('correctedDetected', 0)}")
    except Exception as exc:
        print(f"[RESET] approvalState update skipped: {exc}")
...
def unapproved_reporting_date(row: Dict[str, Any]) -> str:
    """Unapproved date used only for snapshot fallback/reporting helpers."""
    if not is_snapshot_unapproved(row):
        return ""
    return approval_decision_date(row)
...
def same_created_and_decision_day(row: Dict[str, Any]) -> bool:
    ...
    decision_day = unapproved_reporting_date(row)
...
def _approval_day_now() -> str:
    """Coarse 'today' (date only) used as the detected approval day. C4C exposes
    no change timestamp, so the day we first see a ticket as approved is the best
    available approximation; time-of-day is intentionally not stored."""
    return now_iso()[:10]


def _approval_state_date_from_snapshot(row: Dict[str, Any], decision: str) -> str:
    if decision not in {"approved", "unapproved"}:
        return ""
    return approval_decision_date(row)


def _approval_state_time_from_snapshot(row: Dict[str, Any], decision: str, fallback_day: str) -> str:
    if decision not in {"approved", "unapproved"}:
        return clean(fallback_day)
    value = approval_decision_time(row)
    if value:
        return value
    return clean(fallback_day)


def _approval_state_record(tid: str, cur: Dict[str, Any], decision: str, day: str, source: str) -> Dict[str, Any]:
    decision_time = _approval_state_time_from_snapshot(cur, decision, day)
    return {
        "ticketId": tid,
        "ticketName": clean(cur.get("name")),
        "decision": decision,
        "decisionDay": day,
        "decisionTime": decision_time,
        "changedOn": decision_time,
        "approvalDecisionSource": clean(cur.get("approvalDecisionSource") or source),
        "claimApprovedOn": clean(cur.get("claimApprovedOn") or cur.get("claimApprovedOnDateTime")),
        "resolvedOn": clean(cur.get("resolvedOn") or cur.get("resolvedOnDateTime")),
        "changeOnDateTime": clean(cur.get("changeOnDateTime") or cur.get("changedOn") or cur.get("lastUpdateDate")),
        "postingDate": clean(cur.get("postingDate")),
        "status": clean(cur.get("statusText")),
        "claimType": clean(cur.get("claimType")) or ticket_claim_type(cur),
        "po": clean(cur.get("erpPurchaseOrder")),
        "source": source,
        "firstSeenDay": _approval_day_now(),
    }


def update_approval_state(monitor_root: str, snap: Dict[str, Any]) -> Dict[str, int]:
    ...
    detects transitions automatically:
      ...
      - Unapproved is recorded similarly (first-seen day) but never downgrades an
        existing approved record.
    """
    prev_raw = db.reference(f"{monitor_root}/approvalState").get() or {}
    prev = approval_state_dict_by_ticket_id(prev_raw)
    today = _approval_day_now()
    patch: Dict[str, Any] = {}
    newly_approved = 0
    newly_unapproved = 0
    corrected_detected = 0

    for _key, cur in snap.items():
        if not isinstance(cur, dict):
            continue
        tid = clean(cur.get("id"))
        if not tid:
            continue
        decision = clean(cur.get("approvalDecision"))
        if decision not in ("approved", "unapproved"):
            continue

        existing = prev.get(approval_state_storage_key(tid)) or prev.get(safe_key(tid)) or prev.get(tid)
        existing = existing if isinstance(existing, dict) else None
        already_approved = bool(existing) and clean(existing.get("decision")) == "approved"
        already_unapproved = bool(existing) and clean(existing.get("decision")) == "unapproved"
        existing_source = clean(existing.get("source") if existing else "")
        existing_day = date_key(existing.get("decisionDay") if existing else "")
        current_day = _approval_state_date_from_snapshot(cur, decision)
        ...
        if decision == "approved":
            ...
            if already_approved and existing_day == current_day and existing_source == record_source:
                if clean(existing.get("claimType")) == clean(record.get("claimType")) and clean(existing.get("decisionTime")) == clean(record.get("decisionTime")):
                    continue
            patch[approval_state_storage_key(tid)] = record
            if already_approved:
                corrected_detected += 1
            else:
                newly_approved += 1
        else:  # unapproved
            if already_approved:
                continue  # never downgrade an approved record
            record_source = decision_source or ("detected" if not current_day else "legacy_unapproved_backup")
            if already_unapproved:
                if existing_source == "detected" and current_day and (not existing_day or current_day <= existing_day):
                    patch[approval_state_storage_key(tid)] = _approval_state_record(tid, cur, "unapproved", current_day, record_source)
                    corrected_detected += 1
                elif existing_source and record_source and existing_source != record_source and current_day:
                    patch[approval_state_storage_key(tid)] = _approval_state_record(tid, cur, "unapproved", current_day, record_source)
                    corrected_detected += 1
                continue  # never downgrade, and keep first unapproved day
            day = current_day or today
            patch[approval_state_storage_key(tid)] = _approval_state_record(tid, cur, "unapproved", day, record_source)
            newly_unapproved += 1

    if patch:
        db.reference(f"{monitor_root}/approvalState").update(patch)
        db.reference(f"{monitor_root}/approvalStateMeta").update({
            "lastScanDay": today,
            "lastNewlyApproved": newly_approved,
            "lastNewlyUnapproved": newly_unapproved,
            "lastCorrectedDetected": corrected_detected,
        })
    return {"newlyApproved": newly_approved, "newlyUnapproved": newly_unapproved, "correctedDetected": corrected_detected}
...
def compare_and_write(
    ...
    # - no logs are generated for baseline seeding or reconciliation.
    ...
    # Self-rolling approval date line: detect approved/unapproved transitions vs
    # the last scan (and the one-time Excel baseline), stamping the day we first
    # see each transition. No C4C change API and no repeated Excel exports needed.
    try:
        approval_stats = update_approval_state(monitor_root, snap)
        print(f"[COMPARE] approvalState: newlyApproved={approval_stats['newlyApproved']}, "
              f"newlyUnapproved={approval_stats['newlyUnapproved']}, "
              f"correctedDetected={approval_stats.get('correctedDetected', 0)}")
    except Exception as exc:
        print(f"[COMPARE] approvalState update skipped: {exc}")
...
def rebuild_delivery_flow_history(source_root: str, db_url: str, sa_path: str) -> None:
    """Rebuild the delivery-flow history snapshot that the Parts Delivery Flow page reads."""
    script = Path(__file__).resolve().with_name("delivery_flow_aggregator.py")
    if not script.exists():
        print(f"[DELIVERY FLOW] Skipped: missing {script.name}")
        return
    ...
    print(f"[DELIVERY FLOW] Rebuilding history via {script.name} ...")
    result = subprocess.run([sys.executable, str(script), "--once"], env=env)
    if result.returncode != 0:
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--skip-dealer-analytics", action="store_true", help="Rebuild only employee/team analytics; leave dealer analytics unchanged.")
...
def run_once(args: argparse.Namespace) -> None:
    ...
    include_dealer = not args.skip_dealer_analytics
    compare_and_write(
        args.source_root,
        args.monitor_root,
        include_dealer=include_dealer,
        db_url=args.firebase_db_url,
...
def main() -> None:
    ...
    if args.reset_baseline:
        ...
        reset_baseline(
            args.source_root,
            args.monitor_root,
            clear_history=args.clear_history_on_reset,
            db_url=args.firebase_db_url,
            sa_path=args.firebase_sa_path,
        )
        return
